    ...
    *************************************************************/
    ...
    public:
        ...
        bool init(const CaffeParam &param)
        {
			...
			if(param._models_content.empty() && param._weights_content.empty()) {
				...
				CDEBUG_LOG("load models_file :%s", param._models_file.c_str());
				string decrypt_content(param._models_content);
				_caffe_net_ = boost::shared_ptr<caffe::Net<float> >(new Net<float>(param._models_file, decrypt_content, caffe::TEST));
				CDEBUG_LOG("load weights_file :%s", param._weights_file.c_str());
				decrypt_content = param._weights_content;
				_caffe_net_->CopyTrainedLayersFrom(param._weights_file, decrypt_content);
				CDEBUG_LOG("predict model::init quit");
			}
            return true;
        }
